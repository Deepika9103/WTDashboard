...
const Hero = () => {
  ...
  //   const finalText = "your beautiful Journey";
  ...
  //   return (
  //     <Typography variant="body2" sx={{ fontSize: "34px", fontWeight: "bold", mt: 10, mb: 4,  }}>
  //       <span>{typedText}</span>
  //     </Typography>
  //   );
  // };
  const TypingText = () => {
    ...
    useEffect(() => {
      const typeText = async () => {
        ...
        // await new Promise((resolve) => setTimeout(resolve, 500)); // Delay before typing "Journey"
  
        for (let i = 0; i < journeyText.length; i++) {
          setTypedText((prevText) => prevText + journeyText[i]);
          await new Promise((resolve) => setTimeout(resolve, 50));
        }
  
        // No need to wait before sliding in the additional text
        setTypedText((prevText) => prevText + journeyText); 
    ...
    return (
      ...
      <div>
        ...
        <Slide in={typedText.includes(journeyText)} direction="up" timeout={1000}>
